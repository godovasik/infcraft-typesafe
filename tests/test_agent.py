...
def test_quoted_task_text_still_uses_the_llm(monkeypatch):
    ...
    post = Mock(return_value={"output_text": '{"text":"Zurich"}'})
    monkeypatch.setattr(model, "post_json", post)
    context = model.field_context('Fly from "Zurich" to London', page()["actions"][0], page(), [])
    assert model.field_text(context)[0] == "Zurich"
    assert post.call_count == 1
    assert post.call_args.args[0] == "https://api.openai.com/v1/responses"
    assert post.call_args.args[2]["model"] == "gpt-5.6-luna"
    sent = json.loads(post.call_args.args[2]["input"])
...
def test_browser_close_ignores_a_target_already_closed(monkeypatch):
    import jev_ultrafast.browser as browser
    ...
    monkeypatch.setattr(
        browser,
        "cdp",
        Mock(side_effect=RuntimeError("{'code': -32602, 'message': 'No target with given id found'}")),
    )
    b.close()
...
def test_text_helper_rejects_invalid_values(monkeypatch, content):
    monkeypatch.setenv("OPENAI_API_KEY", "test")
    monkeypatch.setattr(model, "post_json", Mock(return_value={"output_text": content}))
