...
from .alchemy import AlchemyAgent
from .model import OPENAI_TEXT_MODEL
from .questions import ALCHEMY_MAX_MODEL_CALLS, MAX_STEPS
...
def response_state():
    if not AGENT:
        return {"mode": "generic", "page": None, "status": "idle", "history": [], "decision": None}
    state = AGENT.snapshot()
    if isinstance(AGENT, AlchemyAgent):
        ...
        state = {**state, "mode": "generic", "scenario": SCENARIO}
    return {
        **state,
        "text_model": OPENAI_TEXT_MODEL,
        "max_steps": MAX_STEPS,
        "max_model_calls": state.get("max_model_calls", MAX_STEPS * 2),
    }
...
def command(name, body):
    ...
    if name == "reset":
        ...
        if scenario not in {"travel", "research", "flights", "alchemy", "alchemy_fixture"}:
        ...
        close_browser()
        SCENARIO = scenario
        if scenario in {"alchemy", "alchemy_fixture"}:
            url = "https://neal.fun/infinite-craft/" if scenario == "alchemy" else f"{ORIGIN}/alchemy-fixture.html"
            AGENT = AlchemyAgent(url, goal, screenshots=True, max_model_calls=ALCHEMY_MAX_MODEL_CALLS)
        else:
            AGENT = Agent(
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        files = {
            ...
            "/alchemy-fixture.html": ("alchemy_fixture.html", "text/html"),
        }
