...
The committed video completes the Google Flights task in **7.073 seconds at 1×**. It starts with one natural-language goal and uses dynamic controls throughout. Jev selects operation + target in one request; the historical recording used Mercury to generate the city strings when TYPE_TEXT was selected. The current helper configuration uses only OpenAI `gpt-5.6-luna`.
